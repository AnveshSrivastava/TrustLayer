"""Groq collector module for the hallucination detector.

This module provides an async wrapper around the Groq API (OpenAI-compatible).
...
def _build_prompt(query: str, context: Optional[str] = None) -> str:
    """Build the prompt text for Groq based on optional context."""
...
def _extract_text(payload: Any) -> str:
    """Extract text from the Groq response payload."""
    if payload is None:
        return ""
...
async def _invoke_with_retry(prompt: str) -> Any:
    """Invoke Groq with one retry and timeout handling."""
    last_exception: Optional[Exception] = None
    for attempt in range(2):
        try:
            return await asyncio.wait_for(_call_groq(prompt), timeout=TIMEOUT_SECONDS)
        except Exception as exc:
            last_exception = exc
            if attempt == 1:
                raise last_exception
    raise RuntimeError("Groq retry logic failed")


class GroqCollector(BaseCollector):
    """Groq model collector.

    This collector interfaces with Groq's API to fetch responses.
    ...
    def model_name(self) -> str:
        """Return the model identifier: 'groq'."""
        return "groq"
    ...
    async def get_response(
        ...
        try:
            ...
            error = str(exc).strip() or "Unknown groq error"
...
async def get_response(query: str, context: Optional[str] = None) -> Dict[str, Any]:
    """Get a normalized response from Groq.

    This function is provided for backward compatibility.
    New code should use GroqCollector directly.
    """
    collector = GroqCollector()
    return await collector.get_response(query, context)


# Auto-register at import time
_groq_collector = GroqCollector()